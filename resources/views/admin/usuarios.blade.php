...
<div class="panel-administracion">
    ...
    <nav class="menu-navegacion">
        {{-- Añadimos el enlace al Dashboard central --}}
        <a href="{{ route('admin.index') }}" class="enlace-menu">Inicio</a>
        
        {{-- Usuarios ahora es el activo en esta vista --}}
        ...
        <table class="tabla-gestion">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    {{-- Corregimos la fecha para que use el campo real de la base de datos --}}
                    <td>{{ $usuario->created_at ? $usuario->created_at->format('d/m/Y') : 'No disponible' }}</td>
                    <td>
                        @if($usuario->tipo_usuario != 'admin')
                            <form action="{{ route('admin.usuarios.destroy', $usuario->id) }}" method="POST" onsubmit="return confirm('¿Estás seguro de eliminar este usuario?')">
                                @csrf
                                @method('DELETE')
